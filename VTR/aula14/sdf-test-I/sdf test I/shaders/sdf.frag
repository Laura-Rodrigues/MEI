...
object map(vec3 p) {
  ...
  object sphere = object(length(p1) - 1, 2); // raio = 1 e cor = 2
...
vec3 getMaterial(int id) {
  ...
  switch(id) {
    ...
    case 2: m = vec3(1.0, 1.0, 0.0); break;
    case 3: m = vec3(1.0, 0.0, 1.0); break;
  }
...
vec3 getLight(vec3 p, vec3 rayDirection, vec3 color) {
  ...
  vec3 specular = specColor * pow(max(0.0, dot(refl, view)), 10);
...
object rayMarch(vec3 rayOrigin, vec3 rayDirection) {
...
vec3 getRayDirection(vec2 fc, vec2 offset) {
  ...
  rayDirection.xy = 2.0 * fc / WindowSize -1.0;
...
void main(){
  ...
  c += getColor(obj, rayDirection); // é preciso a direção do raio para calcular a luz especular
