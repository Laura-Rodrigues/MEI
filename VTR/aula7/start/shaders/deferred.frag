...
void main(){
  ...
  vec3 p = texture(pos, tc).xyz;
  vec2 tc_obj = texture(texCoord, tc).st;

  int c = int(snoise(vec3(0))*5); // 5 é o número de cores que tenho
  ...
  nn = texture(normalMap, tc_obj).xyz * 2 - 1; // ir buscar valor à textura e descompactá-lo
  nn = normalize(tbn * nn);
  ...
  if (intensity > 0.0){
    ...
    float shine = max(0.0, dot(h,nn));
    specular = vec4(1.0) * pow(shine, 80);
