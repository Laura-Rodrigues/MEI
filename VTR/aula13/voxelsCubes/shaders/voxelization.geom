...
layout (rgba8) uniform coherent image3D imageUnit;
...
float pixelDiagonal = 1.0/imageSize(imageUnit).x;
...
void main() {
	...
	// find normal largest component
	float m = max(normal.z, max(normal.y, normal.x));

	// compute screen space position using the "best" camera
	// colorG color codes the camera used
	vec4 screenPos[3];
	if (m == normal.x) {
